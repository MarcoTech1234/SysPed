<?php
    ...
    if($requestData['operacao'] == 'create'){
        ...
        try {
            ...
            $sql = "INSERT INTO VENDA (DATA, SUBTOTAL, DESCONTO, VLRTOTAL, STATUS, ATENDENTE_ID, FPAGAMENTO_ID, CLIENTE_ID	
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if($requestData['operacao'] == 'update'){
        ...
        try {
            // Gerar a querie de inserção de dados no B.D.
            $sql = "UPDATE VENDA SET NOME = ?, LOGIN = ?, SENHA = ?, ACESSO_ID = ? WHERE ID = ?";
            // Iremos preparar a nossa querie para gerar o objeto de inserção ao B.D.
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $requestData['NOME'],
